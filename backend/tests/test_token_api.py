...
def test_create_token(mock_submit, client, app_context):
    ...
    response = client.post(
        "/api/tokens/create", json=token_data, content_type="application/json"
    )
    assert response.status_code == 200
    ...
    assert (
        len(transactions) == 3
    )  # enable_rippling, trust_set, and payment transactions
...
def test_list_tokens(mock_submit, client, app_context):
    ...
    for i in range(2):
        token_data = {
            ...
            "name": f"TestToken{i}",
            "supply": 1000000 * (i + 1),
            "metadata": {"description": f"Test token {i} for unit testing"},
        }
        response = client.post(
            "/api/tokens/create", json=token_data, content_type="application/json"
        )
...
def test_list_transactions(mock_submit, client, app_context):
    ...
    token_data = {
        ...
        "name": "TestToken",
        "supply": 1000000,
        "metadata": {"description": "Test token for unit testing"},
    }
    response = client.post(
        "/api/tokens/create", json=token_data, content_type="application/json"
    )
...
def test_create_token_invalid_wallet(client, app_context):
    ...
    token_data = {
        "wallet": {"classic_address": "invalid_address", "secret": "invalid_seed"},
        "name": "TestToken",
        "supply": 1000000,
        "metadata": {},
    }
